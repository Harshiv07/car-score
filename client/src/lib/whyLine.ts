 ...
 */
export function whyLine(l: ScoredListing): string {
  ...
  const best = scored.filter((c) => STRENGTHS[c.key] && c.frac >= 0.8).sort((a, b) => b.frac - a.frac)[0];
  ...
  if (best) parts.push(STRENGTHS[best.key]);
