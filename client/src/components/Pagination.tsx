export function Pagination({
  ...
  const btn =
    "min-w-9 rounded-lg border px-3 py-1.5 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-40";
  const idle =
    "border-slate-300 text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800";
  const active = "border-cyan-600 bg-cyan-600 text-white";
  ...
  return (
    <nav className="mt-6 flex items-center justify-center gap-1.5" aria-label="Pagination">
      ...
      {nums[0] > 1 && (
        <>
          <button className={`${btn} ${idle}`} onClick={() => onPage(1)}>1</button>
      ...
      {nums[nums.length - 1] < pages && (
        <>
          ...
          <button className={`${btn} ${idle}`} onClick={() => onPage(pages)}>{pages}</button>
        </>
      )}
