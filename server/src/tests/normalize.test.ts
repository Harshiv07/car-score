...
test("dedupe key: VIN wins, else composite of year+make+model+trim+price+dealer", () => {
  const vinKey = dedupeKeyFor({
    vin: "2T3P1RFV8LC081989", year: 2020, make: "Toyota", model: "RAV4",
    trim: "XLE", price: 25988, dealer: "A",
  });
  assert.ok(vinKey.startsWith("vin:"));

  const a = dedupeKeyFor({ vin: null, year: 2020, make: "Toyota", model: "RAV4", trim: "XLE", price: 25988, dealer: "A" });
  const b = dedupeKeyFor({ vin: null, year: 2020, make: "toyota", model: "rav4", trim: "xle", price: 25988, dealer: "a" });
  const c = dedupeKeyFor({ vin: null, year: 2020, make: "Toyota", model: "RAV4", trim: "XLE", price: 25000, dealer: "A" });
  assert.equal(a, b, "composite key is case-insensitive");
  assert.notEqual(a, c, "different price → different key");
});
